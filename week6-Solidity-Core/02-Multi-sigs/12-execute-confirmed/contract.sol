...
contract MultiSig {
    ...
    function isConfirmed(uint _tx) public view txExists(_tx) onlyOwner returns(bool) {
        return transactions[_tx].confirmations > 1;
    }

    function executeTransaction(uint _tx) public txExists(_tx) onlyOwner {
        require(isConfirmed(_tx), "Tx is not confirmed yet");
        Transaction storage transaction = transactions[_tx];
        (bool success, ) = payable(transaction.destination).call{value: transaction.value}("");
